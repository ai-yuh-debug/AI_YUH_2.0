...
def cleanup_inactive_memory():
    """Verifica, sumariza e salva memórias de usuários inativos."""
    now = datetime.now()
    inactive_users = [
...
def main():
    ...
    if missing_vars:
        print(f"ERRO: Faltando variáveis no .env: {', '.join(missing_vars)}")
        return
    ...
    if not gemini_handler.GEMINI_ENABLED or not database_handler.DB_ENABLED:
        print("O bot não pode iniciar devido a um erro na inicialização de um módulo.")
        return
